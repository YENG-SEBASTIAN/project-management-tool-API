...
import {
  ...
  TASK_UPDATE_SUCCESS,
  TASK_UPDATE_FAIL,
  TASK_UPDATE_PARTIAL_REQUEST,
  TASK_UPDATE_PARTIAL_SUCCESS,
  TASK_UPDATE_PARTIAL_FAIL,
  TASK_DELETE_REQUEST,
  TASK_DELETE_SUCCESS,
  TASK_DELETE_FAIL,
...
export const taskUpdatePartialReducer = (state = initialState, action) => {
  switch (action.type) {
    case TASK_UPDATE_PARTIAL_REQUEST:
      return { ...state, loading: true };
    case TASK_UPDATE_PARTIAL_SUCCESS:
      return {
        ...state,
        loading: false,
        tasks: state.tasks.map((task) => (task.id === action.payload.id ? action.payload : task)),
      };
    case TASK_UPDATE_PARTIAL_FAIL:
      return { ...state, loading: false, error: action.payload };
    default:
      return state;
  }
...
const tasksReducer = combineReducers({
  ...
  taskUpdatePartial: taskUpdatePartialReducer,
